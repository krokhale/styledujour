...
class ClothingItemsController < ApplicationController
  ...
  def show
    ...
    if @clothing_item.overall_hcit_score && @clothing_item.price
    ...
    respond_to do |wants|
      ...
      wants.json do
         render :json=>@clothing_item.to_json(:except=>[:activity_object_id, :heir_id, :heir_type, :photo_content_type, :photo_file_name, :photo_file_size, :photo_updated_at],
          :methods=>[:photo_url],:include=>[:heir => {:only=>[:item_url]}]), :status => 200
      end
     end
  end
  ...
  def create
    ...
    if @clothing_item.save
      unless current_user.hcit_items.where("user_asked_clothing_items.clothing_item_id = ?",@clothing_item).first
        ...
        #Point.award(current_user, "HCIT_add_clothing_item") ### Do this on after_create instead
      end
